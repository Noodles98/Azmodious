...
{
	if (task is null || task.GetType() != Task::Type::BUILDER || cast<IBuilderTask>(task).GetBuildType() != Task::BuildType::MEX)
		return false;
	if (IsLegionDef(builderDef))
		return false;

	return HasLegionMexNear(cast<IBuilderTask>(task).GetBuildPos());
}
