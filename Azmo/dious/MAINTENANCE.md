...
- Faction build-chain defences run independently; native cluster-defence requests require a short sustained-contact frontline anchor before the military manager places them.
...
- `script/hard/helper/commander_factory_assist.as`: tracks a new factory whose first constructor was queued at zero existing count. The builder manager gives commanders short, highest-priority guard tasks on that factory until the constructor's unit-finished event, overriding mex and energy work for the full build.
- `script/hard/helper/factory_limit.as`: metal-income factory count caps and pressure-independent switch-income checks. The first T1 and first T2 factory are free; additional T1 factories require 15 metal income each, and additional T2 factories require 20 metal income each. Factory switching also accepts sustainable income scaled by total factory count, so a quiet economy can reach T2 without first accumulating attack-driven army cost or factory-cost metal storage.
- `script/hard/helper/economy_smooth.as`: smoothed economy readings used by economy decisions.
- `script/hard/helper/energy_space_reclaim.as`: tracks finished T1 wind/solar plus advanced solar/converters, then creates late-game builder reclaim tasks when nearby space-hungry base builds need room. Stage 1 clears T1 wind/solar; stage 2 clears advanced solar/T1 converters only after fusion economy is established, skipping while energy is empty or stalling.
- `script/hard/helper/factory_exit_reclaim.as`: tracks finished non-air factories and non-factory structures, then creates high-priority builder reclaim tasks for own buildings whose centers sit in the factory-front exit strip. It reads `CCircuitUnit.GetBuildingFacing()` from the profile DLL and rotates the strip for the factory's actual facing; air factories are ignored because aircraft do not need land exit clearance, and mexes plus static economy infrastructure are exempt so the AI does not reclaim needed economy buildings while trying to clear a factory exit.
...
- `script/hard/helper/frontline_cluster.as`: tracks up to three confirmed frontline pressure anchors per AI. Close pressure merges into the nearest anchor; distant confirmed pressure can become a second anchor. These anchors shape defence positions, fog-probe gating, and positioned committed attack tasks. Native DLL bindings for unit-positioned fight tasks are available (`SFightTask.hasPosition`/`position`). Treat the script-side anchors as a local policy cache for the AI; if the DLL exposes explicit frontline anchorpoint or unit-position data, keep that integration on the native side and use the script helper to decide when to activate or refresh it. Anchor learning now uses observed ground-combat pressure (`ObservePressure`) and no longer retrains from defence request positions. The helper also stores a short-lived last-known enemy pressure position so attack tasks can keep a fallback target after vision is lost.
...
- `script/hard/helper/military_task.as`: role-aware military fight-task policy layered before the engine default task selector. It maps unit roles/attributes to fight tasks such as scout, raid, attack, defend, bomber, artillery, support, AA, AH, and super. Mainline attack-class units stay on committed attack by default, and it reserves a limited share of eligible ground combat units for fog-push scout tasks when no stable frontline anchor is known. Attack positioning first prefers stable frontline anchors, then falls back to a short-lived remembered enemy-pressure position from `frontline_cluster.as`.
...
- `script/hard/helper/maps/default_profiles.as`: Registers the metadata registry.
...
- `script/hard/helper/maps/metadata_profiles.as`: combined registry. It embeds curated/local profiles first, then archive-derived `mapinfo.lua` fallback starts that default to FRONT.
...
- `script/hard/manager/military.as`: role-aware combat task assignment plus defence creation gating; unhandled combat units still fall back to engine defaults. Mainline non-air combat units now feed frontline-pressure observations under active enemy mobile threat, and defence placement consumes stable frontline anchors when available.
...
- `config/hard/block_map.json`: terrain analysis and building footprint/blocking classes. Ground-factory blocker offsets stay centered on the factory X axis and extend forward along +Z; keep every factory variant in the appropriate `fac_*` instance list so placement reserves its exit before builder tasks are created.
...
- T1 land mex entries in the active faction `*BuildChain.json` files keep `"porc": false` but directly queue the faction LLT/light-AA pair, bypassing the generic pressure gate without creating porcupine clusters. Advanced, underwater, and special mex variants remain unprotected by that chain; frontline placement is still shaped through `script/hard/manager/military.as` and `script/hard/helper/frontline_cluster.as`.
...
- Change map/start-position role assignments in `metadata_profiles.as`, default-role behavior, role dispatch, or allowed factory families: `script/hard/helper/role/role.as`
...
- Change actual defence unit order in the faction-specific `config/hard/*BuildChain.json` files unless a direct script selector is added. Frontline defence requests are anchor-gated in `script/hard/manager/military.as`.
...
3. Keep role-specific defensive posture in `GetPreferredFightType()` and `GetDefendPromotePower()`. TECH currently defends more aggressively, AIR defends mainline ground units, SEA has its own defend promotion power, and FRONT has average promote power.
...
7. Tune factory exit cleanup in `script/hard/helper/factory_exit_reclaim.as` via `FACTORY_EXIT_FRONT_OFFSET`, `FACTORY_EXIT_DEPTH`, `FACTORY_EXIT_HALF_WIDTH`, and `FACTORY_EXIT_CENTER_PAD`. The helper rotates its south-facing local strip using `CCircuitUnit.GetBuildingFacing()` from `SkirmishAI.dll`, ignores air factories, and reclaims own non-factory structures blocking the exit before lower-priority builder cleanup helpers run.
...
9. If base layout looks like a single random center blob, reduce `economy.cluster_range` first (for example, from 1000 toward 500-650) before changing block-map footprints; this usually yields smaller, more structured local build clusters.
...
3. Tune the faction-specific `porcupine` configuration for unit order and cluster size; it remains responsible for turret selection.
4. Keep `script/hard/manager/military.as` as the owner that requires a sustained-contact frontline anchor, applies lane-biased placement, and calls `aiMilitaryMgr.DefaultMakeDefence(...)`.
...
- Terrain tuning now combines static block-map tuning, terrain metadata attached to map profile registrations, normalized local height classification, and optional Lua hint overrides via `AiLuaMessage`. Use Lua hints only for live overrides when the current game needs a runtime adjustment; keep the map-profile data as the default source of truth.
- Extra frontline defence requests require a short sustained-contact anchor in `script/hard/manager/military.as`. Natural build-chain defences and actual default defence selection remain controlled by the faction-specific `config/hard/*BuildChain.json` files and `aiMilitaryMgr.DefaultMakeDefence(...)`.
- Military combat task assignment first checks `script/hard/helper/military_task.as` for role/attribute-specific intents such as scout, raid, artillery, support, AA, bomber, and super; unknown or generic units still use `aiMilitaryMgr.DefaultMakeTask(...)`. Mainline attack-class routing should not return `DEFEND` just for positional variety, because that can make winnable fights look like threats to avoid. Role helper `MILITARY_ATTACK_THRESHOLD`, `MILITARY_RAID_MIN_POWER`, and `MILITARY_RAID_AVG_POWER` values control how much combat power pools before squads release, so lower them to reduce army blobbing.
...
- AIR factory selection is two-layered: `script/hard/helper/role/air.as` decides when T1/T2 air plants are allowed and when an existing T1 should promote the next factory pick to advanced air, while `script/hard/helper/role/role.as` applies that preference after the engine default factory choice. This is separate from per-unit `limit` values in `config/hard/*Behaviour.json`. If advanced-air is selected but position-rejected by AIR anchor-distance checks, `script/hard/manager/factory.as` queues a tracked replacement task around the T1 anchor; `builder.as` clears that task handle when the task ends so a failed replacement can be retried.
...
- `default_profiles.as` registers `metadata_profiles.as`, then adds archive fallbacks only when no curated/local profile prefix matches.
