...
- `script/hard/helper/defense.as`: adaptive defence gate helpers modeled as `ShouldBuild...` checks. Generic defence creation remains gated by game time, metal income, and enemy mobile threat. Factory and T1 mex LLT/light-AA coverage bypasses that global gate through explicit faction build chains instead, so quiet games do not continuously open the military defence scheduler.
...
3. Tune adaptive build permission in `script/hard/helper/defense.as`. It exposes `ShouldBuildT1LightAA`, `ShouldBuildT1LightTurret`, `ShouldBuildT2FlakAA`, `ShouldBuildLRPC`, and related helpers. T1 light turrets (LLT) and light anti-air rocket launchers (RL) intentionally return `true` so early scout/leak coverage is not gated.
