...
- `script/hard/helper/energy_space_reclaim.as`: tracks finished T1 wind/solar and creates late-game builder reclaim tasks when nearby space-hungry base builds need room, skipping while energy is empty or stalling.
...
6. Tune late-game T1 wind/solar space reclaim in `script/hard/helper/energy_space_reclaim.as` via `ENERGY_SPACE_RECLAIM_START_FRAME`, `ENERGY_SPACE_RECLAIM_TIMEOUT`, and `ENERGY_SPACE_RECLAIM_RADIUS`. This uses builder reclaim tasks against exact T1 wind/solar unit names and skips while energy is empty or stalling.
