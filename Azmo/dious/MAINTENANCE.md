...
- `script/hard/helper/energy_space_reclaim.as`: tracks finished T1 wind/solar/converters and advanced solar, then creates late-game builder reclaim tasks when nearby space-hungry base builds need room. Stage 1 clears T1 space energy; stage 2 clears advanced solar only for later energy placements, skipping while energy is empty or stalling.
...
6. Tune late-game energy space reclaim in `script/hard/helper/energy_space_reclaim.as` via `ENERGY_SPACE_RECLAIM_START_FRAME`, `ADVANCED_SOLAR_SPACE_RECLAIM_START_FRAME`, `ENERGY_SPACE_RECLAIM_TIMEOUT`, and `ENERGY_SPACE_RECLAIM_RADIUS`. This uses builder reclaim tasks against exact T1 wind/solar/converter names first, then advanced solar for later energy placements, and skips while energy is empty or stalling.
