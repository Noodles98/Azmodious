...
- `script/hard/helper/defense.as`: pressure and affordability policy for extra frontline defence. Faction build-chain defences run independently; native cluster-defence requests require a confirmed frontline anchor before the military manager places them.
...
3. Tune extra-frontline-defence pressure and affordability in `script/hard/helper/defense.as`. This gate does not select a turret; faction-specific `porcupine` configuration remains responsible for the unit order.
4. Keep `script/hard/manager/military.as` as the owner that requires a confirmed frontline anchor, applies lane-biased placement, and calls `aiMilitaryMgr.DefaultMakeDefence(...)`.
...
- Extra frontline defence gating lives in `script/hard/helper/defense.as`; `script/hard/manager/military.as` additionally requires a confirmed frontline anchor. Natural build-chain defences and actual default defence selection remain controlled by the faction-specific `config/hard/*BuildChain.json` files and `aiMilitaryMgr.DefaultMakeDefence(...)`.
