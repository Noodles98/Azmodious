...
- `script/hard/helper/defense.as`: dispatches extra-frontline-defence requests through the active role's adaptive pressure/economy policy. Faction build-chain defences run independently; native cluster-defence requests require a short sustained-contact frontline anchor before the military manager places them.
...
3. Tune role-specific staged extra-frontline-defence pressure and income thresholds in `script/hard/helper/role/*.as`; `script/hard/helper/defense.as` dispatches that policy. This gate does not select a turret; faction-specific `porcupine` configuration remains responsible for the unit order.
...
- Extra frontline defence dispatch lives in `script/hard/helper/defense.as`, with staged role thresholds in `script/hard/helper/role/*.as`; `script/hard/manager/military.as` additionally requires a short sustained-contact frontline anchor. Natural build-chain defences and actual default defence selection remain controlled by the faction-specific `config/hard/*BuildChain.json` files and `aiMilitaryMgr.DefaultMakeDefence(...)`.
