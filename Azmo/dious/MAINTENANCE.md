...
- `script/hard/helper/factory_exit_reclaim.as`: tracks finished factories and non-factory structures, then creates high-priority builder reclaim tasks for own buildings whose centers sit in the factory-front exit strip. It uses the block-map south-facing convention (+Z as front) and an 18-elmo exit depth with a small center pad.
...
7. Tune factory exit cleanup in `script/hard/helper/factory_exit_reclaim.as` via `FACTORY_EXIT_FRONT_OFFSET`, `FACTORY_EXIT_DEPTH`, `FACTORY_EXIT_HALF_WIDTH`, and `FACTORY_EXIT_CENTER_PAD`. The helper assumes the block-map south-facing convention where factory front is +Z, and it reclaims own non-factory structures blocking the exit before lower-priority builder cleanup helpers run.
